...
| `recognized()` | Function | <p>Sets the `Recognizeable` instance's `status` to `'recognized'`.</p><p>You should _only_ call this function after the information you've gathered from events and stored in `metadata` proves that your custom gesture has occurred.</p> | none | none |
...
7. Based on all of the information you've gathered, your handler should make a decision: 
    - Has the custom gesture been recognized? If so, call the `recognized` function.
    - Still not sure, and need to wait for more events? Do nothing—`Recognizeable` will make sure the recent event gets stored safely in the `sequence` array in case you need it later.
...
::: type="info"
Still not sure how to use this workflow to recognize your custom gestures? If you're up for a source dive, you can [check out the Baleada Listenable Gestures source code](https://gitlab.com/baleada/listenable-gestures/-/tree/master/src%2Fhandler-getters) for inspiration.
:::
...
| Stores the constructor's state in a public getter named after the state's type | <ApiDesignSpecCheckmark /> | `sequence`  |
| Has a public method you can use to set a new value for that public getter | <ApiDesignSpecCheckmark /> | `setSequence` |
...
| Has at least one additional getter property that you can't (and shouldn't) set directly | <ApiDesignSpecCheckmark /> | `status`, `metaData`, `lastEvent` |
| Has one or more public methods that expose core functionality | <ApiDesignSpecCheckmark /> | `recognize` |
| Either has no side effects or has side effects that can be cleaned up with a `stop` method | <ApiDesignSpecCheckmark /> |  |
| Uses the sentence template to decide what state type should be accepted by a constructor | <ApiDesignSpecCheckmark /> | "A sequence can be recognized." |
